...
// Performance: Cache for expensive graph data calculations
const graphCache = new Map();
const GRAPH_CACHE_TTL = 30000; // 30 seconds
const MAX_GRAPH_CACHE_SIZE = 30;
...
function buildHeroGraphData(db, options = {}) {
  ...
  // Reduced multiplier from 4x to 1.5x for better performance
  // Cap at 10,000 instead of unlimited to prevent memory issues
  const fetchLimit = Number.isFinite(maxHands) 
    ? Math.max(Math.floor(maxHands * 1.5), 2000)
    : 10000;
