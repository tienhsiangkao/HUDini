...
function computeHeroNetFromJson(hand) {
  ...
  if (Array.isArray(hand.players)) {
    const heroPlayer = hand.players.find((p) => namesEqual(p?.name, heroName));
    if (heroPlayer && typeof heroPlayer.net === 'number' && Number.isFinite(heroPlayer.net)) {
      return Number(heroPlayer.net.toFixed(2));
    }
  }
  let win = 0;
